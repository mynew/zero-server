...
struct VendorItem
{
    VendorItem(uint32 _item, uint32 _maxcount, uint32 _incrtime, uint32 _kalimdorcoins)
        : item(_item), maxcount(_maxcount), incrtime(_incrtime), kalimdorcoins(_kalimdorcoins) {}
...
struct VendorItemData
{
    ...
    void AddItem( uint32 item, uint32 maxcount, uint32 ptime, uint32 kalimdorcoins)
    {
        m_items.push_back(new VendorItem(item, maxcount, ptime,kalimdorcoins));
    }
